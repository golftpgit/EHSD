...
# --- พจนานุกรมรหัสโรคทางระบาดวิทยา (รวบกลุ่มรหัสแม่) ---
DISEASE_DICT = {
...
def load_aggregated_data():
    ...
    # แก้ปัญหา Memory Mac: ใช้ as_index=False และบังคับ .copy() เพื่อให้หน่วยความจำสะอาด
    df_m = df_m.groupby(['Group', 'DIAG_CAT', 'DIAGCODE', 'DIAG_NAME', 'YEAR_MONTH', 'FAMP'], as_index=False)['PATIENT_COUNT'].sum().copy()
    df_a = df_a.groupby(['Group', 'DIAG_CAT', 'DIAGCODE', 'DIAG_NAME', 'YEAR', 'FAMP', 'AGE_GROUP'], as_index=False)['PATIENT_COUNT'].sum().copy()
    df_s = df_s.groupby(['Group', 'DIAG_CAT', 'DIAGCODE', 'DIAG_NAME', 'YEAR', 'FAMP', 'SEX_NAME'], as_index=False)['PATIENT_COUNT'].sum().copy()
...
if df_monthly is None or df_age is None or df_sex is None:
    ...
    # =========================================================
    # 🔍 ส่วนควบคุมตัวกรอง (Hierarchical Filters) แบบเสถียร
    # =========================================================
    ...
    temp_df_1 = df_monthly[df_monthly['Group'].isin(filter_group)].copy()
    
    available_cats = sorted(temp_df_1['DIAG_CAT'].unique().tolist()) if not temp_df_1.empty else []
    ...
    temp_df_2 = temp_df_1[temp_df_1['DIAG_CAT'].isin(filter_cat)].copy()
    
    available_codes = sorted(temp_df_2['DIAGCODE'].unique().tolist()) if not temp_df_2.empty else []
    
    select_all_codes = st.sidebar.checkbox("✅ เลือกทุกรหัสโรค", value=True, key="chk_code")
    if select_all_codes:
        filter_codes = available_codes
        st.sidebar.info("คุณกำลังเลือกตรวจสอบทุกรหัสโรค (ปลดติ๊กเพื่อเลือกเฉพาะโรค)")
    else:
        filter_codes = st.sidebar.multiselect(
            "3. เจาะลึกรหัสโรค (DIAGCODE)", 
            options=available_codes,
            format_func=lambda x: f"{x} : {DISEASE_DICT[x]['name']}",
            key="ms_code"
        )
        if not filter_codes:
            st.sidebar.warning("⚠️ กรุณาเลือกรหัสโรค")

    temp_df_3 = temp_df_2[temp_df_2['DIAGCODE'].isin(filter_codes)].copy() if filter_codes else pd.DataFrame(columns=temp_df_2.columns)
    available_amps = sorted(temp_df_3['FAMP'].dropna().unique().tolist()) if not temp_df_3.empty else []
    
    select_all_amps = st.sidebar.checkbox("✅ เลือกทุกพื้นที่ศึกษา", value=True, key="chk_amp")
    if select_all_amps:
        filter_amps = available_amps
        st.sidebar.info("คุณกำลังเลือกดูทุกอำเภอ (ปลดติ๊กเพื่อเลือกเฉพาะอำเภอ)")
    else:
    ...
    # =========================================================
    # 🧬 สรุปผลการกรองขั้นสุดท้าย (Cross-Filtering) พร้อมแก้ Bug Memory
    # =========================================================
    if not filter_codes or not filter_amps:
        ...
        # บังคับ .copy() ทุกครั้งที่ตัดข้อมูล เพื่อไม่ให้เกิด Segmentation Fault
    ...
    if m_filtered.empty:
        ...
        with col_m1:
            st.metric(label="จำนวนเคสผู้ป่วยรวมสะสม (ครั้ง)", value=f"{total_cases:,}")
        with col_m2:
            st.metric(label="เคสเฉียบพลัน (Acute Exposure Trend)", value=f"{acute_cases:,}")
        with col_m3:
            st.metric(label="เคสเรื้อรังสะสม (Chronic Dust Burden)", value=f"{chronic_cases:,}")
        ...
        with tab_main:
            ...
            # จัดการหน่วยความจำสำหรับตาราง
            pivot_display = m_filtered.groupby(['DIAG_CAT', 'DIAGCODE', 'DIAG_NAME', 'FAMP', 'Group'], as_index=False)['PATIENT_COUNT'].sum().copy()
            pivot_display.columns = ['กลุ่มอาการหลัก', 'รหัสโรค', 'ชื่อโรคทางการแพทย์', 'อำเภอ (FAMP)', 'ประเภทโรค', 'จำนวนผู้เจ็บป่วย (ครั้ง)']
            pivot_sorted = pivot_display.sort_values(by='จำนวนผู้เจ็บป่วย (ครั้ง)', ascending=False).copy()
        ...
        with tab_time:
            ...
            if not m_filtered.empty:
                ...
                # ดักจับ Error กราฟ
                try:
                    time_series = m_filtered.groupby(groupby_cols, as_index=False)['PATIENT_COUNT'].sum().copy()
                    ...
                    if has_enough_data:
                        fig_time = px.line(
                            ...
                            labels={
                                'YEAR_MONTH': 'ปี-เดือน', 
                                'PATIENT_COUNT': 'จำนวนผู้ป่วย (ครั้ง)', 
                                color_col: 'คำอธิบาย'
                            },
                            markers=True
                        )
                        ...
                        st.info("💡 ข้อมูลมีจำนวนเดือนน้อยเกินไปที่จะลากเส้นต่อเนื่องได้ ระบบจึงแสดงผลเป็นจุด (Scatter) แทน")
                        fig_time = px.scatter(
                            time_series, 
                            x='YEAR_MONTH', 
                            y='PATIENT_COUNT', 
                    ...
                    st.warning("⚠️ รูปแบบการกรองแคบเกินไปทำให้ประมวลผลกราฟไม่ได้ ลองปรับลดการแยกเส้นดูครับ")
        ...
        with tab_person:
            ...
            with col_p1:
                ...
                if not a_filtered.empty:
                    age_summary = a_filtered.groupby(['AGE_GROUP'], as_index=False)['PATIENT_COUNT'].sum().copy()
                    ...
                    age_summary = age_summary.sort_values(by='order').copy()
                    
                    st.dataframe(age_summary[['AGE_GROUP', 'PATIENT_COUNT']], use_container_width=True)
            ...
            with col_p2:
                ...
                if not s_filtered.empty:
                    sex_summary = s_filtered.groupby(['SEX_NAME'], as_index=False)['PATIENT_COUNT'].sum().copy()
        ...
        with tab_meta:
            ...
            * **B15 (Acute hepatitis A):** ไวรัสตับอักเสบ เอ
            ...
            st.markdown("---")
            st.subheader("⚙️ ขั้นตอนกระบวนการวิเคราะห์ข้อมูลและการทำ Aggregated Data (Data Pipeline)")
            st.info("""
            **สถาปัตยกรรมการจัดการข้อมูล (Data Governance & ETL Architecture)**
            
            1. **แหล่งที่มาข้อมูลดั้งเดิม (Data Source):** สกัดจากคลังข้อมูล **HDC (Health Data Center)** อ้างอิงสถานะล่าสุด ณ วันที่ 13 กรกฎาคม 2569
            2. **การทำความสะอาดข้อมูล (Data Cleaning):** ระบุพื้นที่ผู้ป่วยที่ชัดเจน และคำนวณอายุที่แม่นยำ
            3. **กลไกการขจัดความซ้ำซ้อน (De-duplication):** เฉพาะโรคกลุ่ม Chronic (J62.8) จะถูกคัดกรองเคสซ้ำรายคนออกไปเพื่อป้องกัน Over-counting Bias
            4. **การสรุปยอด (Data Aggregation & Privacy):** ข้อมูลถูกสรุปรวม (Count) และลบฟิลด์ระบุตัวตน (เช่น CID) ออก 100% ทำให้ปลอดภัยตามหลัก PDPA
            """)
        ...
        st.sidebar.download_button(
            label="ดาวน์โหลดข้อมูลตามฟิลเตอร์ (.csv)",
            data=csv_data,
            file_name="epidemiological_filtered_data.csv",
            mime="text/csv"
        )